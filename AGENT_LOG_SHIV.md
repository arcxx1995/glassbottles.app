# AGENT_LOG_SHIV.md — DevOps Engineer

> Shiv's decisions on CI/CD, Vercel, GitHub Actions, and infra.
...
---
<!-- Shiv appends sessions below this line -->
...
- apps/web/tailwind.config.ts (Cherry's design tokens)
...
- Supabase project ref needed for supabase CLI config — Felix to provide
...
**HANDOFF →**: Felix: supabase/config.toml needs project ref. Run `supabase link --project-ref <ref>` to link local CLI to remote project.
...
| Supabase CLI local | Needs project ref from Felix |
| .env.example | Done |
| Agent dashboard | Done |
